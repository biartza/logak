...
while read z; do
awk -v d1=$(date -d "5 minutes ago" +"%R") -v d2=$(date -d "0 minutes ago" +"%R") '$3 > d1 && $3 < d2 || $0 ~ d2' /var/log/syslog | grep $z | awk '{print $4}' > /tmp/gelakoak.log
	while read p; do
		cat /home/script/gaurkoak.log | grep $p > /dev/null
		if [ $? -ne 0 ]; then # $? returns the exit status of the previous command
    			echo "ez dago"
			echo $p >> /home/script/gaurkoak.log
			echo "<div style='background-color:#97ff9e; padding-left:40%; margin:3px; width:420px;  height:20px;'>" >> /var/www/html/gailuak/$url-$z.html
			echo "Honako gailua piztu da: <a href='"$p".html'>"$p"</a>" >> /var/www/html/gailuak/$url-$z.html
			echo "</br>" >> /var/www/html/gailuak/$url-$z.html
			echo "</div>" >> /var/www/html/gailuak/$url-$z.html
		        echo "<div style='background-color:white; padding:3px; margin:3px; width:420px;  height:20px;'>" >> /var/www/html/gailuak/$url-$z.html
			echo "</br>" >> /var/www/html/gailuak/$url-$z.html
			echo "</div>" >> /var/www/html/gailuak/$url-$z.html
		fi

	done </tmp/gelakoak.log


done </home/script/gelak.info
exit 0
#awk -v d1=$(date -d "5 minutes ago" +"%R") -v d2=$(date -d "0 minutes ago" +"%R") '$3 > d1 && $3 < d2 || $0 ~ d2' /var/log/syslog | grep 2F | awk '{print $4}' > /tmp/gailuak.log
#url=$( date -d "5 minutes ago" +"%F" )
#echo $url
#while read p; do
